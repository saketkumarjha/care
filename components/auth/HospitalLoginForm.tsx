...
import { getHospitalById } from "@/lib/api/hospital";
function LoginPage() {
  ...
  const {
    ...
    defaultValues: {
      email: "",
      password: "",
      rememberMe: false, // Match the schema's optional default
    },
  });
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      // If login is successful and we have a hospitalId and token
      if (loginResponse.token && loginResponse.hospitalId) {
        // Store token in localStorage or cookies
        if (data.rememberMe) {
          localStorage.setItem("authToken", loginResponse.token);
          localStorage.setItem("hospitalId", loginResponse.hospitalId);
          console.log("Auth data stored in localStorage");
        } else {
          // Use sessionStorage if not "remember me"
          sessionStorage.setItem("authToken", loginResponse.token);
          sessionStorage.setItem("hospitalId", loginResponse.hospitalId);
        ...
        try {
          ...
          console.log(
            "Fetching hospital details with ID:",
            loginResponse.hospitalId
          );
          const hospitalResponse = await getHospitalById(
            loginResponse.hospitalId,
            loginResponse.token
          );
          ...
          if (hospitalResponse.success) {
            console.log("Hospital data:", hospitalResponse.data);
        ...
        console.error(
          "Missing token or hospitalId in response:",
          loginResponse
        );
